    ...
    <div class="space-y-6">
        <div
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white">Data Pesanan</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Kelola seluruh pesanan layanan dari pelanggan</p>
                </div>
                <div class="flex items-center flex-wrap gap-2.5">
                        ...
                        <div id="sort-dropdown-menu"
                            ...
                            @foreach($sortOptions as $key => $label)
                                @php
                                    ...
                                    if ($key !== 'terbaru')
                                        $qp['sort'] = $key;
                                    $sortUrl = url('/admin/pesanan') . (count($qp) ? '?' . http_build_query($qp) : '');
                                @endphp
                                <a href="{{ $sortUrl }}"
                    ...
                    @php 
                        $isFiltered = request()->hasAny(['q', 'layanan_id', 'status', 'tanggal_dari', 'tanggal_sampai', 'tanggal_mulai', 'tanggal_selesai']); 
                        $activeCount = collect([
                            request('q'),
                            request('layanan_id'),
                            request('status'),
                            request('tanggal_dari') ?? request('tanggal_mulai')
                        ])->filter()->count();
                    @endphp
                    <div class="relative" id="filter-popover-wrapper">
                        ...
                        <div id="filter-popover-card"
                            ...
                            <form method="GET" action="{{ url('/admin/pesanan') }}" id="main-filter-form">
                                @if(request('sort'))
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                ...
                                                <input type="date" id="filter-tanggal-dari" name="tanggal_dari"
                                                    value="{{ request('tanggal_dari', request('tanggal_mulai')) }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="date" id="filter-tanggal-sampai" name="tanggal_sampai"
                                                    value="{{ request('tanggal_sampai', request('tanggal_selesai')) }}"
                                                    class="w-full px-2.5 py-2 text-xs rounded-lg border border-slate-200 bg-slate-50 text-slate-700 focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 focus:bg-white transition-all">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="text" id="filter-keyword" name="q" value="{{ request('q') }}"
                                                placeholder="ID Pesanan, Pelanggan, HP..."
                                ...
                                <div class="flex items-center gap-2 px-4 py-3 bg-slate-50 border-t border-slate-100">
                                    <a href="{{ url('/admin/pesanan') . (request('sort') ? '?sort=' . request('sort') : '') }}"
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if(request('q'))
                        <span
                            ...
                            <a href="{{ url('/admin/pesanan') . '?' . http_build_query(request()->except('q')) }}"
                    ...
                    @if(request('layanan_id'))
                        ...
                        @if($selectedLayanan)
                            <span
                                ...
                                <a href="{{ url('/admin/pesanan') . '?' . http_build_query(request()->except('layanan_id')) }}"
                                    class="w-4 h-4 rounded-full bg-teal-200/70 hover:bg-teal-200 flex items-center justify-center text-teal-800 transition-colors"
                    ...
                    @if(request('status'))
                        ...
                        <span
                            ...
                            <a href="{{ url('/admin/pesanan') . '?' . http_build_query(request()->except('status')) }}"
                                class="w-4 h-4 rounded-full bg-teal-200/70 hover:bg-teal-200 flex items-center justify-center text-teal-800 transition-colors"
                    ...
                    @endif
                    @if(request('tanggal_dari') || request('tanggal_sampai') || request('tanggal_mulai') || request('tanggal_selesai'))
                        @php
                            $tDari = request('tanggal_dari', request('tanggal_mulai'));
                            $tSampai = request('tanggal_sampai', request('tanggal_selesai'));
                        @endphp
                        <span
                            class="inline-flex items-center gap-2 pl-3 pr-1.5 py-1 rounded-full bg-teal-50 text-teal-700 font-medium text-xs">
                            <span>{{ $tDari }}{{ $tSampai ? ' – ' . $tSampai : '' }}</span>
                            <a href="{{ url('/admin/pesanan') . '?' . http_build_query(request()->except(['tanggal_dari', 'tanggal_sampai', 'tanggal_mulai', 'tanggal_selesai'])) }}"
                                class="w-4 h-4 rounded-full bg-teal-200/70 hover:bg-teal-200 flex items-center justify-center text-teal-800 transition-colors"
                    ...
                    <tbody
                        ...
                        @forelse($orders as $order)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                                <td class="px-6 py-4 text-center font-bold text-slate-400 align-middle">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4 font-extrabold text-blue-600 dark:text-blue-400 whitespace-nowrap align-middle">
                                    {{ $order->id_pesanan }}
                                </td>
                                <td class="px-6 py-4 font-semibold text-slate-900 dark:text-white align-middle">
                                    ...
                                    <div class="space-y-1.5 min-w-[200px] max-w-sm">
                                        @foreach($order->pemesananLayanan as $item)
                                            <div class="flex items-start gap-2">
                                                <span class="inline-block w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-blue-400 mt-1.5 shrink-0"></span>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap align-middle">
                                    <a href="{{ url('/admin/detail-pesanan/' . $order->id) }}"
                                        class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/40 dark:text-blue-300 font-bold text-xs transition-all border border-blue-200 dark:border-blue-800 shadow-2xs">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
