...
@section('title', 'Dokumentasi Proyek')
@section('page_title', 'Dokumentasi & Portofolio Proyek')
...
@section('content')
    <div class="space-y-6">
        ...
        <div
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white">Dokumentasi Proyek</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Kelola foto portofolio dan dokumentasi proyek pengerjaan</p>
                </div>
                    ...
                    <div class="relative" id="sort-dropdown-wrapper">
                        <button type="button" id="sort-dropdown-btn"
                            ...
                            <span class="text-slate-600 dark:text-slate-200">
                                ...
                                @elseif(request('sort') == 'jenis_asc') Jenis Layanan
                                @else Sort by
                                @endif
                            </span>
                        ...
                        <div id="sort-dropdown-menu"
                            ...
                            @foreach($sortOptions as $key => $label)
                                @php
                                    ...
                                    if ($key !== 'terbaru')
                                        $qp['sort'] = $key;
                                    $sortUrl = url('/admin/dokumentasi-proyek') . (count($qp) ? '?' . http_build_query($qp) : '');
                                @endphp
                                <a href="{{ $sortUrl }}"
            ...
            <div class="relative max-w-md">
                ...
                <input type="text" id="search-dok" placeholder="Cari nama atau jenis layanan proyek..."
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="py-3.5 px-6 text-center w-28">Foto Layanan</th>
                            <th class="py-3.5 px-6 text-left">Nama Layanan</th>
                            <th class="py-3.5 px-6 text-left">Jenis Layanan</th>
                    ...
                    <tbody
                        ...
                        @forelse($dokumentasis as $index => $item)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" data-searchable
                                ...
                                <td class="px-6 py-4 font-bold text-slate-800 dark:text-slate-200">{{ $item->nama_layanan }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button
                                            onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->jenis_layanan) }}', '{{ addslashes($item->nama_layanan) }}', '{{ asset($item->gambar) }}')"
                                        ...
                                        <button
                                            onclick="openDeleteModal({{ $item->id }}, '{{ addslashes($item->nama_layanan) }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-sm font-semibold text-slate-400 dark:text-slate-500">Belum ada
                                            dokumentasi proyek.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
                        <tr id="empty-search-row" style="display: none;">
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm font-semibold text-slate-400 dark:text-slate-500">Dokumentasi tidak
                                        ditemukan.</p>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div
                    ...
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Tambah Dokumentasi Baru</h3>
                ...
                <form action="{{ url('/admin/dokumentasi-proyek') }}" method="POST" enctype="multipart/form-data"
                    class="p-6 space-y-5">
                    @csrf
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Jenis
                            Layanan <span class="text-red-500">*</span></label>
                        <input type="text" name="jenis_layanan" required placeholder="Contoh: Jasa Service / Jasa Perizinan"
                    ...
                    <div>
                        <label
                            ...
                            Layanan / Proyek <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_layanan" required placeholder="Contoh: Pemasangan Panel Listrik..."
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Foto
    ...
    <div id="modal-edit" class="fixed inset-0 z-[99] hidden">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div
                    class="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-800 rounded-t-2xl z-10">
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Edit Dokumentasi Proyek</h3>
                ...
                <form id="form-edit" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Jenis
                            Layanan <span class="text-red-500">*</span></label>
                        <input type="text" name="jenis_layanan" id="edit-jenis" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Nama
                            Layanan / Proyek <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_layanan" id="edit-nama" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Ganti
                            Foto <span class="text-slate-400 text-[10px] normal-case">(kosongkan jika tidak
                                diubah)</span></label>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus Dokumentasi?</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Anda yakin ingin menghapus foto proyek
                        <strong id="delete-nama" class="text-slate-800 dark:text-slate-200"></strong>? Tindakan ini tidak
                        bisa
                        dibatalkan.</p>
    ...
    <script>
        ...
        function openEditModal(id, jenis, nama, gambarSrc) {
            document.getElementById('form-edit').action = '/admin/dokumentasi-proyek/' + id;
            document.getElementById('edit-jenis').value = jenis;
            document.getElementById('edit-nama').value = nama;

            const preview = document.getElementById('edit-preview');
            const previewContainer = document.getElementById('edit-preview-container');
            if (gambarSrc && !gambarSrc.includes('/undefined') && !gambarSrc.endsWith('/')) {
                preview.src = gambarSrc;
        ...
        function openDeleteModal(id, nama) {
            document.getElementById('form-hapus').action = '/admin/dokumentasi-proyek/' + id;
            document.getElementById('delete-nama').textContent = nama;
            openModal('modal-hapus');
        }

        document.addEventListener('DOMContentLoaded', function () {
            const searchInput = document.getElementById('search-dok');
            ...
            searchInput.addEventListener('input', function () {
                ...
                rows.forEach(row => {
                    ...
                    const combined = (nama + ' ' + jenis).toLowerCase();
